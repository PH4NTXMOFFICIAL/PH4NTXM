...
LONE WOLF: The exposed hardware persona is regenerated per session with more variation and remains internally coherent for that session.
...
LINUX / WINDOWS: Reported cores and RAM follow seeded device-class and SKU profiles, with bounded jitter and RAM limited by usable host memory.  
LONE WOLF: Uses independent session state and broader device-class ranges within hardware constraints.
