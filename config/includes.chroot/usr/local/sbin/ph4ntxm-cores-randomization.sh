...
case "${SKU,,}" in
    *x1-carbon* | *xps-13* | *zenbook* | *gram* | *surface*)
        BASE_CORES=8
        BASE_RAM=16
        ;;
    *xps-15* | *blade* | *titan* | *zbook* | *legion* | *predator* | *origin* | *omen* | *studio*)
        BASE_CORES=12
        BASE_RAM=32
        ;;
    *ideapad* | *aspire* | *vivobook* | *inspiron* | *flash*)
        BASE_CORES=4
        BASE_RAM=8
        ;;
    *nuc* | *mini*)
...
if ((TEMP_RAM >= 64)); then
    FINAL_RAM=64
elif ((TEMP_RAM >= 32)); then
    FINAL_RAM=32
elif ((TEMP_RAM >= 16)); then
    FINAL_RAM=16
elif ((TEMP_RAM >= 8)); then
    FINAL_RAM=8
elif ((TEMP_RAM >= 4)); then
    FINAL_RAM=4
elif ((TEMP_RAM >= 2)); then
    FINAL_RAM=2
else FINAL_RAM=1; fi
