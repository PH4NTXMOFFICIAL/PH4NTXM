...
use crate::packet::{
    ...
    IcmpInfo, Meta, PacketTransformationResult, QuotedMeta, TcpInfo, UdpInfo, ICMPV4_ERRORS,
    ICMPV6_ECHO, ICMPV6_ERRORS, ICMPV6_MLD, ICMPV6_ND, IPPROTO_ICMP, IPPROTO_ICMPV6, IPPROTO_TCP,
    IPPROTO_UDP,
};
...
impl Engine {
    ...
    fn transform_tcp(
        ...
        if !outbound && info.syn && info.ack && !self.tcp_flows[&key].delta_in_ready {
            ...
            if !info.options.iter().any(|option| option.kind == 3) {
                state.original_window_scale_out = 0;
                state.target_window_scale_out = 0;
            }
        }
        {
            let state = self.tcp_flows.get(&key).expect("flow checked above");
        ...
        if outbound {
            if info.syn && !info.ack {
                ...
                && state_snapshot.original_window_scale_out != state_snapshot.target_window_scale_out
            {
    ...
    fn get_or_create_echo(
        &mut self,
        meta: &Meta,
        identifier: u16,
    ) -> PacketTransformationResult<EchoLocalKey> {
