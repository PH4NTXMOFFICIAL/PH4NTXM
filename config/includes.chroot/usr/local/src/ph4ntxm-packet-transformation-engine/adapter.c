...
extern struct Engine *ph4ntxm_packet_transformation_engine_new(uint8_t mode,
                                                               const uint8_t *seed,
                                                               size_t seed_length);
extern void ph4ntxm_packet_transformation_engine_free(struct Engine *engine);
extern int32_t ph4ntxm_packet_transformation_engine_process(
    struct Engine *engine, const uint8_t *input, size_t input_length, uint8_t outbound,
    uint32_t interface, uint8_t checksum_not_ready, uint8_t *output,
    size_t output_capacity, size_t *output_length, uint8_t *error,
...
static void log_drop(struct runtime *runtime, const char *message)
{
    ...
    if (now >= runtime->last_error_log &&
        now - runtime->last_error_log < ERROR_LOG_INTERVAL_SECONDS) {
        return;
    }
    runtime->last_error_log = now;
    fprintf(stderr,
            "ph4ntxm-packet-transformation-engine-native: packet dropped: %.160s\n",
            message);
}
...
static int packet_callback(struct nfq_q_handle *queue, struct nfgenmsg *message,
                           struct nfq_data *data, void *opaque)
{
    ...
    if (header == NULL) {
        fprintf(
            stderr,
            "ph4ntxm-packet-transformation-engine-native: missing NFQUEUE packet header\n");
        return -1;
    }
    uint32_t packet_id = ntohl(header->packet_id);
    struct packet_metadata *metadata = &runtime->current;
    if (!metadata->valid || metadata->queue_number != context->queue_number ||
        metadata->packet_id != packet_id ||
        metadata->hardware_protocol != ntohs(header->hw_protocol) ||
        metadata->hook != header->hook) {
        fprintf(
            stderr,
    ...
    if (payload_length <= 0 || payload == NULL ||
        (uint32_t)payload_length != metadata->captured_length) {
    ...
    } else if ((metadata->skb_info & ~(NFQA_SKB_CSUMNOTREADY | NFQA_SKB_GSO |
                                       NFQA_SKB_CSUM_NOTVERIFIED)) != 0) {
        drop_reason = "unknown NFQUEUE skb metadata";
    } else {
        uint8_t version = payload[0] >> 4;
        uint16_t expected_protocol = version == 4   ? ETH_P_IP
                                     : version == 6 ? ETH_P_IPV6
                                                    : 0;
        uint8_t expected_hook =
            context->outbound ? NF_INET_LOCAL_OUT : NF_INET_PRE_ROUTING;
        if (expected_protocol == 0 ||
            metadata->hardware_protocol != expected_protocol) {
    ...
    if (drop_reason == NULL) {
        uint32_t interface =
            context->outbound ? nfq_get_outdev(data) : nfq_get_indev(data);
        int32_t result = ph4ntxm_packet_transformation_engine_process(
            runtime->engine, payload, (size_t)payload_length, context->outbound ? 1 : 0,
            interface, (metadata->skb_info & NFQA_SKB_CSUMNOTREADY) != 0 ? 1 : 0,
            runtime->output_buffer, MAX_IP_PACKET_LENGTH, &output_length, error,
            sizeof(error));
        if (result == 1) {
        ...
        drop_reason =
            error[0] == 0 ? "native core rejected packet" : (const char *)error;
        fatal = result < 0;
    }
...
static int extract_metadata(const struct nlmsghdr *netlink_header,
                            struct packet_metadata *metadata)
{
    ...
    if (nfq_nlmsg_parse(netlink_header, attributes) < 0 ||
        attributes[NFQA_PACKET_HDR] == NULL || attributes[NFQA_PAYLOAD] == NULL) {
        fprintf(
            stderr,
    ...
    const struct nfqnl_msg_packet_hdr *packet_header =
...
static int verify_queue_bindings(uint32_t expected_port_id)
{
    ...
    while (fgets(line, sizeof(line), status) != NULL) {
        ...
        int fields = sscanf(line, "%u %u %u %u %u %llu %llu %llu %u", &queue_number,
                            &port_id, &queued, &copy_mode, &copy_range, &queue_dropped,
                            &userspace_dropped, &sequence, &format_sentinel);
        (void)queued;
        (void)sequence;
        if (fields < 9) {
            fclose(status);
        ...
        if (queue_number == INBOUND_QUEUE || queue_number == OUTBOUND_QUEUE) {
            if (port_id != expected_port_id || copy_mode != NFQNL_COPY_PACKET ||
                copy_range < EFFECTIVE_COPY_RANGE || queue_dropped != 0 ||
                userspace_dropped != 0 || format_sentinel != 1) {
                fprintf(
                    stderr,
                    ...
                    "format-sentinel=%u\n",
                    queue_number, port_id, expected_port_id, copy_mode, copy_range,
                    queue_dropped, userspace_dropped, format_sentinel);
                fclose(status);
                errno = EIO;
                return -1;
            }
...
static int notify_systemd(const char *message)
{
    ...
    socklen_t destination_length = (socklen_t)(offsetof(struct sockaddr_un, sun_path) +
                                               length + (address[0] == '@' ? 0 : 1));
...
static int initialize_runtime(struct runtime *runtime, uint8_t mode,
                              const uint8_t seed[32])
{
    ...
    if (runtime->receive_buffer == NULL || runtime->output_buffer == NULL ||
        runtime->engine == NULL || runtime->handle == NULL) {
        return -1;
    }
    runtime->inbound = (struct queue_context){
        .runtime = runtime, .queue_number = INBOUND_QUEUE, .outbound = false};
    runtime->outbound = (struct queue_context){
        .runtime = runtime, .queue_number = OUTBOUND_QUEUE, .outbound = true};
    runtime->inbound_handle = nfq_create_queue(runtime->handle, INBOUND_QUEUE,
                                               packet_callback, &runtime->inbound);
    runtime->outbound_handle = nfq_create_queue(runtime->handle, OUTBOUND_QUEUE,
                                                packet_callback, &runtime->outbound);
    if (runtime->inbound_handle == NULL || runtime->outbound_handle == NULL ||
        configure_queue(runtime->inbound_handle) != 0 ||
        configure_queue(runtime->outbound_handle) != 0) {
        return -1;
    }
    ...
    if (getsockname(descriptor, (struct sockaddr *)&local, &local_length) != 0 ||
        local.nl_family != AF_NETLINK || local.nl_pid == 0) {
        return -1;
    }
    runtime->netlink_port_id = local.nl_pid;
    int socket_buffer = 8 * 1024 * 1024;
#ifdef SO_RCVBUFFORCE
    if (setsockopt(descriptor, SOL_SOCKET, SO_RCVBUFFORCE, &socket_buffer,
                   sizeof(socket_buffer)) != 0) {
        return -1;
    }
#else
    if (setsockopt(descriptor, SOL_SOCKET, SO_RCVBUF, &socket_buffer,
                   sizeof(socket_buffer)) != 0) {
        return -1;
    }
#endif
...
static int event_loop(struct runtime *runtime)
{
    ...
    for (;;) {
        ...
        if (ready > 0 && (event.revents & POLLIN) != 0) {
            struct iovec vector = {.iov_base = runtime->receive_buffer,
            ...
            while (remaining != 0) {
                ...
                if (message_length < sizeof(struct nlmsghdr) ||
                ...
                if (nfq_handle_packet(runtime->handle, (char *)header,
                                      (int)header->nlmsg_len) < 0) {
                ...
                size_t consumed =
                    aligned_length <= remaining ? aligned_length : message_length;
                cursor += consumed;
                remaining -= consumed;
            }
        }
        ...
        if (now < last_maintenance ||
            ...
            if (ph4ntxm_packet_transformation_engine_maintenance(runtime->engine) !=
                0) {
                errno = EFAULT;
                return -1;
            }
...
int main(int argc, char **argv)
{
    if (argc == 2 && strcmp(argv[1], "--self-test") == 0) {
        return ph4ntxm_packet_transformation_engine_self_test() == 0 ? EXIT_SUCCESS
                                                                     : EXIT_FAILURE;
    }
    if (argc != 1) {
    ...
    if (secure_read_ascii("/run/ph4ntxm/mode", mode_text, sizeof(mode_text)) != 0 ||
        secure_read_ascii("/run/ph4ntxm/persona_seed", seed_text, sizeof(seed_text)) !=
            0 ||
        decode_seed(seed_text, seed) != 0) {
        fprintf(
            stderr,
            "ph4ntxm-packet-transformation-engine-native: invalid required state\n");
    ...
    if (notify_ready() != 0) {
        perror(
            "ph4ntxm-packet-transformation-engine-native: readiness notification failed");
        destroy_runtime(&runtime);
        return EXIT_FAILURE;
    }
