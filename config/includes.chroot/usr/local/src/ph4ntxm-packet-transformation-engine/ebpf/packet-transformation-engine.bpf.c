...
const volatile struct ph4ntxm_packet_transformation_engine_guard_config
    ph4ntxm_config SEC(".rodata.ph4ntxm") = {};
...
{
    return candidate[0] == ph4ntxm_config.mac[0] &&
           candidate[1] == ph4ntxm_config.mac[1] &&
           candidate[2] == ph4ntxm_config.mac[2] &&
           candidate[3] == ph4ntxm_config.mac[3] &&
           candidate[4] == ph4ntxm_config.mac[4] &&
           candidate[5] == ph4ntxm_config.mac[5];
}
...
static long zero_bytes_step(__u32 index, void *opaque)
{
    ...
    if (index >= context->length)
        return 1;
    if (load_mac_bytes(context->skb, context->offset + index, &value, sizeof(value)) <
            0 ||
        value != 0) {
        context->failed = 1;
        return 1;
                                     ...
                                     __u32 maximum)
{
    ...
    if (length > maximum || bpf_loop(maximum, zero_bytes_step, &context, 0) < 0 ||
        context.failed)
        return 0;
    return 1;
}
...
static long udp_checksum_step(__u32 index, void *opaque)
{
    ...
    if (byte_offset + 1U >= context->length)
        return 1;
    if (load_mac_bytes(context->skb, context->offset + byte_offset, &word,
                       sizeof(word)) < 0) {
        context->failed = 1;
        return 1;
    }
...
static __noinline int udp_checksum_valid(struct __sk_buff *skb, __u32 udp_offset,
                                         const struct ipv4_header *ip, __u16 udp_length,
                                         int rewrite)
{
    ...
    struct udp_checksum_context context = {
        ...
        .offset = udp_offset,
        .length = udp_length,
        .sum = (source >> 16) + (source & 0xffffU) + (destination >> 16) +
               (destination & 0xffffU) + IPPROTO_UDP + udp_length,
    };

    if (bpf_loop((DHCP_MAX_IP_LENGTH - 20U) / 2U, udp_checksum_step, &context, 0) < 0 ||
        context.failed)
        return 0;
    if ((udp_length & 1U) != 0) {
        __u8 last;
...
static __always_inline int ipv4_layout_valid(struct __sk_buff *skb,
                                             __u32 network_offset, int exact,
...
static __always_inline int ipv6_layout_valid(struct __sk_buff *skb,
                                        ...
                                        const __u8 destination_mac[6])
{
    ...
    if (operation == 1)
        return bytes_are_zero(skb, offset + 18U, 6U, 6U)
                   ? PACKET_TRANSFORMATION_ENGINE_DROP_REASON_COUNT
                   : PACKET_TRANSFORMATION_ENGINE_DROP_ARP_TARGET;
    if (operation == 2)
...
static __always_inline int validate_eapol(struct __sk_buff *skb, __u32 offset)
{
    ...
    if (load_mac_bytes(skb, offset, header, sizeof(header)) < 0 || header[0] < 1 ||
        header[0] > 3 || header[1] > 4)
        return 0;
...
static __always_inline int validate_client_identifier(struct __sk_buff *skb,
                                                      __u32 offset, __u8 length)
{
    __u8 value[7];
...
static __always_inline int validate_hostname(struct __sk_buff *skb, __u32 offset,
                                             __u8 length)
{
    if (length != ph4ntxm_config.hostname_length)
        return 0;
#pragma clang loop unroll(full)
    for (__u32 index = 0; index < PH4NTXM_PACKET_TRANSFORMATION_ENGINE_HOSTNAME_MAX;
         index++) {
        __u8 value;
...
static __always_inline int validate_vendor(struct __sk_buff *skb, __u32 offset,
                                           __u8 length)
{
    __u8 value[8];

    if (length != sizeof(value) ||
        load_mac_bytes(skb, offset, value, sizeof(value)) < 0)
        return 0;
    if (ph4ntxm_config.mode == PH4NTXM_PACKET_TRANSFORMATION_ENGINE_MODE_LINUX)
        return value[0] == 'd' && value[1] == 'h' && value[2] == 'c' &&
               value[3] == 'l' && value[4] == 'i' && value[5] == 'e' &&
               value[6] == 'n' && value[7] == 't';
    return value[0] == 'M' && value[1] == 'S' && value[2] == 'F' && value[3] == 'T' &&
           value[4] == ' ' && value[5] == '5' && value[6] == '.' && value[7] == '0';
}

static __always_inline int validate_parameter_request_list(struct __sk_buff *skb,
                                                           __u32 offset, __u8 length)
{
    __u8 value[6];
    const __u8 native[17] = {1,  2,  6,  12, 15,  26,  28,  121, 3,
                             33, 40, 41, 42, 119, 249, 252, 17};

    if (length == sizeof(native)) {
#pragma clang loop unroll(full)
        for (__u32 index = 0; index < sizeof(native); index++) {
            __u8 byte;
            if (load_mac_bytes(skb, offset + index, &byte, 1) < 0 ||
                byte != native[index])
                return 0;
        }
        return 1;
...
static long dhcp_option_step(__u32 iteration, void *opaque)
{
    ...
    if (context->cursor >= context->length)
        return 1;
    if (load_mac_bytes(context->skb, context->offset + context->cursor, &code,
                       sizeof(code)) < 0) {
        context->failed = 1;
        return 1;
    }
    ...
    if (context->cursor >= context->length ||
        load_mac_bytes(context->skb, context->offset + context->cursor, &option_length,
    ...
    case 12:
        if ((context->seen & DHCP_SEEN_HOSTNAME) != 0 ||
            !validate_hostname(context->skb, context->offset + context->cursor,
                               option_length))
            context->failed = 1;
    ...
    case 53: {
        ...
        if ((context->seen & DHCP_SEEN_MESSAGE_TYPE) != 0 || option_length != 1 ||
            load_mac_bytes(context->skb, context->offset + context->cursor, &value,
                           sizeof(value)) < 0 ||
    ...
    case 55:
        if ((context->seen & DHCP_SEEN_PARAMETER_LIST) != 0 ||
            !validate_parameter_request_list(
                context->skb, context->offset + context->cursor, option_length))
            context->failed = 1;
    ...
    case 57: {
        ...
        if ((context->seen & DHCP_SEEN_MAX_SIZE) != 0 || option_length != 2 ||
            load_mac_bytes(context->skb, context->offset + context->cursor, &size, 2) <
                0 ||
            bpf_ntohs(size) < 576)
            context->failed = 1;
    ...
    case 60:
        if ((context->seen & DHCP_SEEN_VENDOR) != 0 ||
            !validate_vendor(context->skb, context->offset + context->cursor,
                             option_length))
            context->failed = 1;
                                         ...
                                         const struct dhcp_options_context *context)
{
    ...
    __u8 profile[18] = {60,  8,  'd', 'h', 'c', 'l', 'i', 'e', 'n',
                        't', 55, 6,   1,   28,  3,   15,  6,   12};
    __u8 requested[6] = {50, 4};
    __u8 server[6] = {54, 4};
    __u8 end = 255;
    __u32 hostname_length = ph4ntxm_config.hostname_length;
    if (hostname_length == 0 ||
        hostname_length > PH4NTXM_PACKET_TRANSFORMATION_ENGINE_HOSTNAME_MAX)
        return 0;
#pragma clang loop unroll(full)
    ...
    if (ph4ntxm_config.mode == PH4NTXM_PACKET_TRANSFORMATION_ENGINE_MODE_WINDOWS) {
        const __u8 windows_profile[18] = {60,  8,  'M', 'S', 'F', 'T', ' ', '5', '.',
                                          '0', 55, 6,   1,   3,   6,   12,  15,  28};
    ...
    if (bpf_skb_change_tail(skb, offset + sizeof(zero), 0) < 0 ||
        bpf_skb_store_bytes(skb, offset, zero, sizeof(zero), BPF_F_INVALIDATE_HASH) <
            0 ||
        bpf_skb_store_bytes(skb, offset, prefix, sizeof(prefix),
                            BPF_F_INVALIDATE_HASH) < 0)
        return 0;
...
#pragma clang loop unroll(full)
    ...
    if (bpf_skb_store_bytes(skb, cursor, profile, sizeof(profile),
                            BPF_F_INVALIDATE_HASH) < 0)
        return 0;
...
static __noinline int rewrite_dhcp_ipv4(struct __sk_buff *skb, __u32 network_offset)
{
    ...
    ip.ttl = ph4ntxm_config.mode == PH4NTXM_PACKET_TRANSFORMATION_ENGINE_MODE_WINDOWS
                 ? 128
                 : 64;
                                                       ...
                                                       __u32 network_offset)
{
    ...
    if (udp.source != bpf_htons(DHCP_CLIENT_PORT) ||
        ...
        return PACKET_TRANSFORMATION_ENGINE_DROP_DHCP_UDP;
    if (load_mac_bytes(skb, bootp_offset, bootp, sizeof(bootp)) < 0 || bootp[0] != 1 ||
        bootp[1] != 1 || bootp[2] != 6 || bootp[3] != 0)
    ...
    udp.checksum = 0;
    if (bpf_skb_store_bytes(skb, udp_offset, &udp, sizeof(udp), BPF_F_INVALIDATE_HASH) <
            0 ||
...
int ph4ntxm_guard(struct __sk_buff *skb)
{
    ...
    if (ph4ntxm_config.magic != PH4NTXM_PACKET_TRANSFORMATION_ENGINE_CONFIG_MAGIC ||
        ...
        ph4ntxm_config.mode > PH4NTXM_PACKET_TRANSFORMATION_ENGINE_MODE_WINDOWS ||
        ph4ntxm_config.hostname_length == 0 ||
        ph4ntxm_config.hostname_length >
            PH4NTXM_PACKET_TRANSFORMATION_ENGINE_HOSTNAME_MAX ||
...
#pragma clang loop unroll(full)
    ...
    if (skb->mark == PH4NTXM_PACKET_TRANSFORMATION_ENGINE_PROVENANCE_MARK) {
        if (skb->gso_segs > 1 ||
            (protocol == ETH_P_IP &&
             !ipv4_layout_valid(skb, network_offset, 1, &marked_ipv4)) ||
        ...
        if (protocol == ETH_P_IP && marked_ipv4.protocol == IPPROTO_UDP) {
            struct udp_header udp;
            if (load_mac_bytes(skb, network_offset + sizeof(marked_ipv4), &udp,
                               sizeof(udp)) < 0)
                return record_drop(PACKET_TRANSFORMATION_ENGINE_DROP_MARKED_LAYOUT);
            if (udp.source == bpf_htons(DHCP_CLIENT_PORT) &&
                udp.destination == bpf_htons(DHCP_SERVER_PORT)) {
    ...
    if (protocol == ETH_P_ARP) {
        ...
        return reason == PACKET_TRANSFORMATION_ENGINE_DROP_REASON_COUNT
                   ? TC_ACT_OK
                   : record_drop(reason);
    }
    if (protocol == EAPOL_ETHERTYPE)
        return validate_eapol(skb, network_offset)
                   ? TC_ACT_OK
                   : record_drop(PACKET_TRANSFORMATION_ENGINE_DROP_EAPOL);
