...
#define PH4NTXM_PACKET_TRANSFORMATION_ENGINE_EBPF_OBJECT                               \
...
static int print_loaded_identity(
    int program_fd,
...
static int read_text(const char *path, char *output, size_t capacity)
{
    ...
    while (length < (size_t)metadata.st_size) {
        ssize_t result =
            read(descriptor, output + length, (size_t)metadata.st_size - length);
...
static int
current_config(const char *interface,
...
static int attach_program(const char *interface)
{
    ...
    if (current_config(interface, &config) != 0) {
        fprintf(
            stderr,
            "ph4ntxm-packet-transformation-engine-loader: failed to read interface configuration\n");
        return -1;
    }
    open_options.sz = sizeof(open_options);
    stage = "open eBPF object";
    object = bpf_object__open_file(PH4NTXM_PACKET_TRANSFORMATION_ENGINE_EBPF_OBJECT,
                                   &open_options);
...
static int
read_attached_config(__u32 program_id,
...
static int read_verified_program(const char *interface,
                                 struct program_identity *identity)
{
    ...
    if (error != 0) {
        ...
        fprintf(
            stderr,
            "ph4ntxm-packet-transformation-engine-loader: failed to query Packet Transformation Engine filter\n");
        return -1;
    }
    ...
    if (memcmp(&attached, &expected, sizeof(expected)) != 0) {
        ...
        fprintf(
            stderr,
            "ph4ntxm-packet-transformation-engine-loader: attached configuration mismatch\n");
        return -1;
    }
    return 0;
}
...
static int print_drop_counters(const char *interface)
{
    ...
    for (__u32 map_index = 0; map_index < program_info.nr_map_ids; map_index++) {
        ...
        if (bpf_obj_get_info_by_fd(map_fd, &map_info, &map_info_length) == 0 &&
            ...
            strncmp((const char *)map_info.name, "ph4ntxm_drops",
                    sizeof(map_info.name)) == 0) {
            for (__u32 reason = 0;
                 reason < PACKET_TRANSFORMATION_ENGINE_DROP_REASON_COUNT; reason++) {
                __u64 value;
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2 && strcmp(argv[1], "--self-test") == 0)
        return sizeof(struct ph4ntxm_packet_transformation_engine_guard_config) == 48
                   ? EXIT_SUCCESS
                   : EXIT_FAILURE;
    if (argc != 3 ||
        (strcmp(argv[1], "attach") != 0 && strcmp(argv[1], "verify") != 0 &&
         strcmp(argv[1], "identity") != 0 && strcmp(argv[1], "stats") != 0) ||
        strlen(argv[2]) == 0 || strlen(argv[2]) >= IFNAMSIZ ||
        strchr(argv[2], '/') != NULL || setrlimit(RLIMIT_CORE, &core_limit) != 0) {
