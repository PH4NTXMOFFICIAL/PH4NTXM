    ...
    while cursor < len(options):
        ...
        if kind == 2:
            ...
            if (
                not linux_mode
                or not syn
                or not (option_length == 2 or 6 <= option_length <= 18)
                or (option_length - 2) % 2
            ):
...
def encode_tcp_key(key):
    ...
    return (
        ...
        + struct.pack("!I", scope)
        + left[0]
        + struct.pack("!H", left[1])
        + right[0]
        + struct.pack("!H", right[1])
    )
...
def translate_sack(value, delta):
    ...
    for cursor in range(2, len(value), 8):
        ...
        output.extend(
            struct.pack(
                "!II", (left + delta) & 0xFFFFFFFF, (right + delta) & 0xFFFFFFFF
            )
        )
    return bytes(output)
...
def transform_tcp(raw, meta, info, direction):
    ...
    with STATE_LOCK:
        ...
        if (
            ...
            and info["syn"]
            and info["ack"]
            and not state["delta_in_ready"]
        ):
            state["delta_in"] = (
                derived_int("tcp-isn-in", state["nonce"]) - sequence
            ) & 0xFFFFFFFF
            state["delta_in_ready"] = True
            if 3 not in tcp_option_kinds(info["options"]):
                state["orig_wscale_out"] = 0
                state["target_wscale_out"] = 0
        if (
            direction == "in"
            and not state["delta_in_ready"]
            and not (info["rst"] and info["ack"])
        ):
        ...
        if direction == "out":
            struct.pack_into(
                "!I", header, 4, (sequence + state["delta_out"]) & 0xFFFFFFFF
            )
            if info["ack"]:
                acknowledgement = struct.unpack_from("!I", header, 8)[0]
                struct.pack_into(
                    "!I", header, 8, (acknowledgement - state["delta_in"]) & 0xFFFFFFFF
                )
        else:
            struct.pack_into(
                "!I", header, 4, (sequence + state["delta_in"]) & 0xFFFFFFFF
            )
            if info["ack"]:
                acknowledgement = struct.unpack_from("!I", header, 8)[0]
                struct.pack_into(
                    "!I", header, 8, (acknowledgement - state["delta_out"]) & 0xFFFFFFFF
                )
        header[12] = ((20 + len(options)) // 4) << 4
        if direction == "out":
            if info["syn"] and not info["ack"]:
                struct.pack_into(
                    "!H", header, 14, 64240 if MODE == "windows" else 29200
                )
            elif (
                not info["syn"]
                and state["orig_wscale_out"] != state["target_wscale_out"]
            ):
...
def remove_echo_locked(local_key):
    ...
    if state is not None:
        ECHO_BY_WIRE.pop(
            echo_wire_key(
                state["family"], state["scope"], state["remote"], state["wire_id"]
            ),
            None,
        )
...
def get_or_create_echo(meta, identifier):
    ...
    for attempt in range(65536):
        ...
        wire_key = echo_wire_key(
            meta["family"], meta.get("scope", 0), meta["dst"], wire_id
        )
    ...
    state = {
        ...
        "local_id": identifier,
        "wire_id": wire_id,
        "sequence_delta": derived_int(
            "icmp-echo-sequence", wire_id, FLOW_GENERATION, bits=16
        ),
        "payload_key": derived_int(
            "icmp-echo-payload", wire_id, FLOW_GENERATION, bits=128
        ).to_bytes(16, "big"),
        "last": time.monotonic(),
        "local_key": local_key,
    }
...
def transform_echo(raw, meta, icmp_type, direction):
    ...
    if (meta["family"] == 4 and icmp_type == 8) or (
        meta["family"] == 6 and icmp_type == 128
    ):
        if direction != "out":
            return raw
        with STATE_LOCK:
            ...
            output[offset + 8 :] = xor_echo_payload(
                raw[offset + 8 :], state, wire_sequence
            )
        return bytes(output)
    if direction != "in":
        return raw
    with STATE_LOCK:
        state = ECHO_BY_WIRE.get(
            echo_wire_key(meta["family"], meta.get("scope", 0), meta["src"], identifier)
        )
        if state is None:
...
def reverse_quoted_tcp_options(output, offset, available, state, direction):
    ...
    while cursor < end:
        ...
        if kind == 8 and option_length == 10 and MODE == "linux":
            ...
            kind == 5
            and option_length >= 10
            and option_length <= 34
            and (option_length - 2) % 8 == 0
        ):
            if direction == "in":
                if not state["delta_in_ready"]:
            ...
            for block in range(cursor + 2, cursor + option_length, 8):
                ...
                struct.pack_into(
                    ...
                    output,
                    block,
                    (left + delta) & 0xFFFFFFFF,
                    (right + delta) & 0xFFFFFFFF,
                )
        cursor += option_length
...
def repair_complete_quoted_checksum(output, quote_start, quoted):
    ...
    if quoted["protocol"] == IPPROTO_TCP:
        ...
        value = transport_checksum(
            bytes(output[quote_start:end]), quoted, bytes(segment)
        )
        ...
        struct.pack_into(
            ...
            segment,
            2,
            transport_checksum(bytes(output[quote_start:end]), quoted, bytes(segment)),
        )
...
def translate_quoted_echo(output, quote_start, quoted, direction):
    ...
    with STATE_LOCK:
        state = ECHO_BY_WIRE.get(
            echo_wire_key(
                quoted["family"], quoted.get("scope", 0), quoted["dst"], wire_id
            )
        )
        if state is None:
        ...
        struct.pack_into(
            ...
            offset + 4,
            state["local_id"],
            (wire_sequence - state["sequence_delta"]) & 0xFFFF,
        )
        quoted_end = quote_start + quoted["total_length"]
        if quoted_end <= len(output):
            output[offset + 8 : quoted_end] = xor_echo_payload(
                bytes(output[offset + 8 : quoted_end]), state, wire_sequence
            )
    repair_complete_quoted_checksum(output, quote_start, quoted)
...
def next_ipid(meta):
    ...
    return derived_int(
        "linux-ipid", counter, meta["src"], meta["dst"], meta["protocol"], bits=16
    )
...
def finalize_packet(raw, meta, udp_zero_checksum=False):
    ...
    if meta["protocol"] == IPPROTO_TCP:
        ...
        segment[2:4] = b"\x00\x00"
        struct.pack_into(
            "!H", segment, 2, transport_checksum(bytes(output), meta, bytes(segment))
        )
    else:
...
def main():
    ...
    threading.Thread(
        ...
        daemon=True,
        name="ph4ntxm-packet-transformation-engine-cleanup",
    ).start()
    inbound = NetfilterQueue()
    outbound = NetfilterQueue()
    try:
        inbound.bind(
            INBOUND_QUEUE, modify_inbound, max_len=QUEUE_LENGTH, range=QUEUE_COPY_RANGE
        )
        outbound.bind(
            OUTBOUND_QUEUE,
            modify_outbound,
            max_len=QUEUE_LENGTH,
            range=QUEUE_COPY_RANGE,
        )
