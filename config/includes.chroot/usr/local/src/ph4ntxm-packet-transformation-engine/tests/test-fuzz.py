...
def validate_output(packet, outbound):
    ...
    if family == 4:
        if (
            len(packet) < 20
            or packet[0] != 0x45
            or struct.unpack_from("!H", packet, 2)[0] != len(packet)
        ):
        ...
        if len(packet) < 40 or 40 + struct.unpack_from("!H", packet, 4)[0] != len(
            packet
        ):
    ...
    if protocol == 6:
        if len(segment) < 20 or not 20 <= (segment[12] >> 4) * 4 <= min(
            60, len(segment)
        ):
        ...
        if (
            received != 0
            and pseudo_checksum(family, source, destination, protocol, segment) != 0
        ):
        ...
        if (
            len(segment) < 8
            or pseudo_checksum(family, source, destination, protocol, segment) != 0
        ):
