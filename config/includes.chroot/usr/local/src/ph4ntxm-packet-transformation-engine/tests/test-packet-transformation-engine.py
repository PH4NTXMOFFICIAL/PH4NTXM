...
PYTHON_REFERENCE_ENGINE = ROOT / "tests/test-reference-engine.py"
NATIVE_LIBRARY = Path(
    os.environ.get(
...
def load_oracle(mode):
    ...
    source = source.replace(
        "from netfilterqueue import NetfilterQueue", "class NetfilterQueue:\n    pass"
    )
...
class Native:
    def __init__(self, mode):
        ...
        self.library.ph4ntxm_packet_transformation_engine_free.argtypes = [
            ctypes.c_void_p
        ]
        ...
        self.engine = self.library.ph4ntxm_packet_transformation_engine_new(
            0 if mode == "linux" else 1, seed, 32
        )
        if not self.engine:
...
def ipv4(
    source, destination, protocol, payload, identifier=0x1234, flags=0x4000, ttl=64
):
...
def pseudo_checksum(family, source, destination, protocol, segment):
    if family == 4:
        ...
        pseudo = socket.inet_pton(socket.AF_INET6, source) + socket.inet_pton(
            socket.AF_INET6, destination
        )
...
def udp(family, source, destination, sport, dport, payload):
    segment = bytearray(
        struct.pack("!HHHH", sport, dport, 8 + len(payload), 0) + payload
    )
...
def tcp(
    ...
    struct.pack_into(
        "!H", segment, 16, pseudo_checksum(family, source, destination, 6, segment)
    )
    return bytes(segment)
...
def icmp_echo(icmp_type, identifier, sequence, payload):
    segment = bytearray(
        struct.pack("!BBHHH", icmp_type, 0, 0, identifier, sequence) + payload
    )
...
def icmpv6_echo(source, destination, icmp_type, identifier, sequence, payload):
    ...
    struct.pack_into(
        "!H", segment, 2, pseudo_checksum(6, source, destination, 58, segment)
    )
    return bytes(segment)
...
def run_udp_vectors(mode):
    ...
    try:
        ...
        wire_udp = assert_pair(
            oracle, native, ipv4("192.0.2.10", "198.51.100.20", 17, segment), "out"
        )
        error = ipv4(
            "198.51.100.1", "192.0.2.10", 1, icmp_error(3, 3, wire_udp), flags=0
        )
        ...
        modern_error = ipv6(
            router6, local6, 58, icmpv6_error(router6, local6, 4, 10, wire6)
        )
...
def run_echo_vectors(mode):
    ...
    try:
        request = ipv4(
            ...
            "198.51.100.20",
            1,
            icmp_echo(8, 0x2345, 9, b"echo-payload"),
            flags=0,
        )
        ...
        error = ipv4(
            "198.51.100.1", "192.0.2.10", 1, icmp_error(3, 1, wire_request), flags=0
        )
        assert_pair(oracle, native, error, "in")
        ...
        struct.pack_into(
            ...
            wire_icmp6,
            2,
            pseudo_checksum(6, destination6, source6, 58, wire_icmp6),
        )
...
def run_tcp_handshake(mode):
    ...
    try:
        syn = ipv4(
            ...
            remote,
            6,
            tcp(4, local, remote, sport, dport, local_isn, 0, 0x02, 64240, syn_options),
        )
        ...
        if mode == "linux":
            synack_options = (
                ...
                + b"\x04\x02"
                + b"\x08\x0a"
                + struct.pack("!II", 5000, exposed_timestamp[0])
                + b"\x01\x03\x03\x07"
            )
        else:
            synack_options = (
                struct.pack("!BBH", 2, 4, 1460) + b"\x04\x02\x01\x03\x03\x08\x00\x00"
            )
        synack = ipv4(
            remote,
            local,
        ...
        if mode == "linux":
            outbound_sack_options = (
                ...
                + b"\x05\x0a"
                + struct.pack(
                    "!II", visible_remote_sequence + 1, visible_remote_sequence + 33
                )
            )
        else:
            outbound_sack_options = (
        ...
        if mode == "linux":
            ...
            wire_ack_timestamp = timestamp(
                wire_acknowledgement[40 : 20 + wire_ack_header_length]
            )[0]
        ...
        error = ipv4(
            "198.51.100.1", local, 1, icmp_error(3, 3, wire_acknowledgement), flags=0
        )
        assert_pair(oracle, native, error, "in")
        sack_error = ipv4(
...
def run_drop_vectors(mode):
    ...
    try:
        ...
        try:
            ...
            raise AssertionError(
                "invalid checksum was accepted without offload metadata"
            )
        repaired = native.process(bad_checksum, "out", 2, checksum_not_ready=True)
        if (
            checksum(repaired[:20]) != 0
            or pseudo_checksum(4, "192.0.2.10", "198.51.100.20", 17, repaired[20:]) != 0
        ):
...
def run_local_vectors(mode):
    ...
    try:
        ...
        if transformed != packet:
            raise AssertionError(
                "loopback-DNAT TCP packet received external transformation"
            )
        ...
        if transformed != packet:
            raise AssertionError(
                "loopback-DNAT zero-checksum UDP packet was not preserved"
            )
    finally:
        native.close()


def run_tcp_window_scaling(mode):
    for family, local, remote, wrap, offset in (
        (4, "192.0.2.10", "198.51.100.20", ipv4, 20),
        (6, "2001:db8::10", "2001:db8::20", ipv6, 40),
    ):
        oracle = load_oracle(mode)
        native = Native(mode)

        def packet(outbound, sequence, acknowledgement, flags, window, options=b""):
            source, destination = (local, remote) if outbound else (remote, local)
            sport, dport = (49000, 443) if outbound else (443, 49000)
            return wrap(
                source,
                destination,
                6,
        ...
        try:
            local_isn = 10000
            for original_scale in (7, 0, 8, 14):
                for peer_scale in (None, 0, 7, None):
                    local_isn += 100
                    options = b"\x02\x04\x05\xb4\x04\x02\x01\x03\x03" + bytes(
                        (original_scale, 0, 0)
                    )
                    syn = packet(True, local_isn, 0, 0x02, 64240, options)
                    wire_syn = assert_pair(oracle, native, syn, "out")
                    expected_options = (
                        b"\x02\x04"
                        + struct.pack("!H", 1460 if family == 4 else 1440)
                        + b"\x01\x03\x03\x08\x01\x01\x04\x02"
                        if mode == "windows"
                        else options
                    )
                    if wire_syn[offset + 20 :] != expected_options:
                        raise AssertionError("TCP SYN persona options changed")
                    expected_window = 64240 if mode == "windows" else 29200
                    actual_window = struct.unpack_from("!H", wire_syn, offset + 14)[0]
                    if actual_window != expected_window:
                        raise AssertionError("TCP SYN persona window changed")
                    wire_isn = struct.unpack_from("!I", wire_syn, offset + 4)[0]
                    peer_options = b"\x02\x04\x05\xb4\x04\x02\x00\x00"
                    if peer_scale is not None:
                        peer_options = options[:-3] + bytes((peer_scale, 0, 0))
                    synack = packet(
                        False, 20000, wire_isn + 1, 0x12, 65432, peer_options
                    )
                    local_synack = assert_pair(oracle, native, synack, "in")
                    if struct.unpack_from("!H", local_synack, offset + 14)[0] != 65432:
                        raise AssertionError("TCP SYN-ACK window was not preserved")
                    peer_isn = struct.unpack_from("!I", local_synack, offset + 4)[0]

                    for window in (32000, 0, 1, 65535):
                        ack = packet(True, local_isn + 1, peer_isn + 1, 0x10, window)
                        wire_ack = assert_pair(oracle, native, ack, "out")
                        actual = struct.unpack_from("!H", wire_ack, offset + 14)[0]
                        expected = window
                        if mode == "windows" and peer_scale is not None:
                            expected = min(65535, (window << original_scale) // 256)
                        if actual != expected:
                            raise AssertionError(
                                "TCP window scaling mismatch "
                                f"({mode}, IPv{family}, local WS {original_scale}, "
                                f"peer WS {peer_scale}, input {window}): "
                                f"{actual} != {expected}"
                            )

                    retransmitted_syn = assert_pair(oracle, native, syn, "out")
                    if retransmitted_syn[offset:] != wire_syn[offset:]:
                        raise AssertionError("TCP SYN retransmission changed")
                    if assert_pair(oracle, native, synack, "in") != local_synack:
                        raise AssertionError("TCP SYN-ACK retransmission changed")
                    ack = packet(True, local_isn + 1, peer_isn + 1, 0x10, 32000)
                    wire_ack = assert_pair(oracle, native, ack, "out")
                    expected = 32000
                    if mode == "windows" and peer_scale is not None:
                        expected = min(65535, (32000 << original_scale) // 256)
                    actual = struct.unpack_from("!H", wire_ack, offset + 14)[0]
                    if actual != expected:
                        raise AssertionError(
                            "TCP window changed after handshake retransmission "
                            f"({mode}, IPv{family}, local WS {original_scale}, "
                            f"peer WS {peer_scale}): {actual} != {expected}"
                        )
        finally:
            native.close()
...
def main():
    if not PYTHON_REFERENCE_ENGINE.is_file() or not NATIVE_LIBRARY.is_file():
        raise SystemExit(
            "build the release native library before running differential tests"
        )
    for mode in ("linux", "windows"):
        run_udp_vectors(mode)
        run_echo_vectors(mode)
        run_tcp_handshake(mode)
        run_tcp_window_scaling(mode)
