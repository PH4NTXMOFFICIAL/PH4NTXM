...
The Linux/Windows one-shot service follows and requires its cores generator and runs before the display manager. The script requires readable `cores_env`, a nonempty `persona_seed` and a positive `PH4_REPORTED_CORES` value.
