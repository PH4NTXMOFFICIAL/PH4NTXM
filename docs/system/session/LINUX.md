...
Linux is selected for the boot session before physical network preparation. The mode file and normal-mode marker select the shared normal service chain. The desktop edition is a separate visual choice.
...
The kernel network profile uses ordinary TTL/hop limit 64 and a lower ephemeral-port range. Required sysctl writes are read back. The Packet Transformation Engine adds coordinated packet-level handling, with nftables queues and physical-interface protection enforcing its path.

DNS uses local Unbound and the configured Quad9 and Mullvad TLS endpoints. Browser policy disables its separate DoH path so the prepared resolver remains relevant. The normal Firefox wrapper validates runtime identity files and builds the selected mode's preferences in a private runtime profile.
